...
(`v0.1.0-alpha.2` after `v0.1.0-alpha.1`) or the bare `v0.1.0` for stable. A
hand-pushed `v*` tag comes through here too, so both paths are held to the same
rules.

The rules, and what each one is protecting:

  * The shape is `vX.Y.Z`, `vX.Y.Z-alpha.N` or `vX.Y.Z-beta.N` and nothing
    else. Anything else is not on the ladder and cannot be ordered against the
    tags that are.
  * A tag is never cut twice, and never at or below an existing tag of the same
    core. An alpha cut after a beta sorts under the beta, so /releases/latest
    goes on pointing at the beta, the release notes' baseline never moves past
    it, and the RPM refuses to upgrade to it.
  * A snapshot of version.go's base has to outrank the tag being cut, or the
    snapshot channel strands (see version-precedence.py: the snapshot RPM stops
    upgrading and the snapshot release page starts claiming to be newer than it
    is). For an alpha or beta that means the base core is at least the tag's
    core; for a final it means the base has already been bumped past it. The
    bump is a commit on main and this workflow will not make one, so it is
    asked for up front, when it costs a merge rather than a stranded channel.
...
Reads:
  ...
  BASE       version.go's Version string, e.g. 0.1.0-dev
...
unset), for the jobs that follow:
  ...
  tag          v0.1.0-alpha.1
  rpm_version  0.1.0~alpha.1, since an RPM version cannot carry "-" and "~"
               sorts before the base the way a prerelease should
  prerelease   true or false: whether GitHub should flag the release. Only once
               a final has shipped. A flagged release leaves /releases/latest,
               and release-notes.py takes its baseline from exactly that - so
               while every release is an alpha, flagging one would leave the
               generator with no baseline at all and every release body would
               come out of its fallback path.
...
def parse_tag(tag: str) -> tuple[str, str, int] | None:
    ...
    A final is ("0.1.0", "stable", 0). Anything off the ladder, the rolling
    `snapshot` tag included, is None and plays no part in any decision here.
    """
    match = TAG.match(tag)
    if not match:
        return None
...
def problems(tag: str, tags: list[str], base: str) -> list[str]:
    ...
    for other in sorted(
        ...
        if other != tag:
            found.append(
                f"{other} already exists and {tag} sorts at or below it, so /releases/latest "
                f"would go on pointing at {other} and the RPM would refuse to upgrade to {tag}."
            )
    ...
    if compare_versions(snapshot, tag) <= 0:
        if channel == STABLE:
            ...
            found.append(
                ...
                f"snapshot channel would strand the moment this ships. Bump version.go and wails.json "
            ...
            found.append(
                ...
                f"below this release and the snapshot channel would strand. Bump version.go and "
...
def flag_prerelease(tag: str, latest: str) -> bool:
    ...
    leaves /releases/latest, which is where release-notes.py takes its baseline
    from and what the snapshot workflow compares itself against. While every
    release is an alpha, flagging one would leave both with nothing to read.
    """
    parsed = parse_tag(tag)
