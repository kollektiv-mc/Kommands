...
//
//   tokens.source.json  ->  src/styles/tokens.css
//
...
//
// Konnekt's frontend/scripts/gen-tokens.mjs implements the same contract against a
// byte-identical source and emits two further outputs (a TypeScript table for its
// runtime skin editor, and a plain :root sheet for its marketing site). Kommands
// needs neither, so this emits the stylesheet alone. The validation and the three
// Tailwind-v4 details below are carried over intact — each guards a mistake that
...
interface TokenSource {
  version: number
  color: Record<string, Record<string, { dark: ColorValue; light?: ColorValue | null }>>
  type: {
    size: { unit: string; scale: Record<string, number> }
    family: Record<string, string[]>
  }
  space: unknown
  radius: { unit: string; scale: Record<string, number> }
  border: { unit: string; scale: Record<string, number> }
  motion: {
    duration: { unit: string; scale: Record<string, number> }
...
const UTILITY_ALIAS: Record<string, string> = {
  'bg-base': 'canvas',
...
// Four sides, not x/y — every call site wants one edge, never a pair.
...
// silently shadows the font-size utility of the same name. This bit Konnekt once
// with `base`; the guard is here so it cannot happen again from the JSON side.
...
function validate(src: TokenSource) {
  ...
  for (const group of ['type', 'space', 'radius', 'border', 'motion'] as const) {
...
function css(color: ColorValue): string {
  if (color.alpha == null) return color.hex
  return `rgba(${channels(color.hex).replaceAll(' ', ', ')}, ${color.alpha})`
}

function scalar(value: number, unit: string): string {
  return `${value}${unit}`
}

function fontStack(families: string[], all: Record<string, string[]>): string {
  return families
    .map((family) => {
 ...
 *
 * Produced by scripts/gen-tokens.ts from tokens.source.json, which is vendored
 * from kollektiv/design/tokens.json. Hand edits are reverted by the next run and
 * never reach Konnekt, which derives its own token layer from the same source.
 *
 * To change a value: edit kollektiv/design/tokens.json, run its
 * scripts/sync-tokens.sh, then \`pnpm gen:tokens\` here.
 */`

function emitCss(src: TokenSource): string {
  const lines: string[] = []
  ...
  push(`   what lets a runtime theme change retheme by overriding --bg-base. */`)
  push(`@theme inline {`)
  ...
  const emitTheme = (selector: string, mode: 'dark' | 'light') => {
    push()
    push(`${selector} {`)
    for (const [group, tokens] of Object.entries(src.color)) {
      for (const [name, token] of Object.entries(tokens)) {
...
let src: TokenSource
try {
  src = JSON.parse(raw) as TokenSource
} catch (error) {
  fail(`tokens.source.json is not valid JSON: ${(error as Error).message}`)
}

validate(src)

mkdirSync(dirname(fileURLToPath(CSS_OUT)), { recursive: true })
writeFileSync(CSS_OUT, emitCss(src))

console.log('gen-tokens: wrote src/styles/tokens.css')
