...
const LOADERS = {
  '1.21.1': {
    ...
    fingerprints: () => import('./generated/1.21.1/fingerprints.json'),
...
const fingerprintCache = new Map<string, Promise<Readonly<Record<string, string>>>>()

/**
 * The structural fingerprint of every definition in a version, by definition id.
 *
 * Its own file rather than a field on each definition, and loaded separately from the
 * definitions themselves, because the caller that needs it most cannot afford them: a
 * dashboard of saved commands has to tell a stale tree from a live one, and
 * `structureState` wants the whole definition to answer. That is 560 KB of skeletons
 * to check a hash — the exact load a saved command caches its `preview` to avoid. This
 * is ~1.6 KB gzipped.
 *
 * Generated by `pnpm gen:fingerprints` from this same catalogue, so the index and
 * `fingerprintOf` cannot disagree; `fingerprints.test.ts` asserts it over every
 * definition rather than trusting the generator.
 */
export function loadFingerprints(
  version: VersionDefinition,
): Promise<Readonly<Record<string, string>>> {
  const cached = fingerprintCache.get(version.id)
  if (cached) return cached
  const promise = loadersFor(version)
    .fingerprints()
    .then((m) =>
      readPayload<Readonly<Record<string, string>>>(
        m.default,
        'fingerprints',
        version.id,
        `${version.id}/fingerprints.json`,
      ),
    )
  fingerprintCache.set(version.id, promise)
  return promise
}
